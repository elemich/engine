...
static const W32MSG win32Messages[] = 
{
	{0, "WM_NULL" },
	{1, "WM_CREATE" },
	{2, "WM_DESTROY" },
...
static const char* findMsg(unsigned int code)
{
	for(size_t i=0;i<sizeof(win32Messages)/8;i++)
	{
		if(code==win32Messages[i].code)
			return win32Messages[i].msg;
	}
...
/*
	const int pixelformatattriblist[49] = 
	{
	...
	FILE* f=fopen("opengl.txt","wt");
	for(int k=0;k<200;k++)
	{
		if(wglGetPixelFormatAttribivARB(hdc,k,0,41,pixelformatattriblist,ivalues))
		{
			const const char *strings[41] = 
			{
				"WGL_NUMBER_PIXEL_FORMATS_ARB",
				"WGL_DRAW_TO_WINDOW_ARB",
				"WGL_DRAW_TO_BITMAP_ARB",   
